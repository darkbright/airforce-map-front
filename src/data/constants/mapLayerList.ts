...
// 지도를 추가하거나 삭제하고자 하는 경우, 아래와 같은 형식으로 배열에 추가/삭제 할 것.
...
export interface MapLayerListType {
	...
	// 유저를 위한 한글 이름
	title: string;
	// 최초 사이트 로드 시 해당 지도를 default로 로드할지 여부
	default: boolean;
	// 지도 샘플 이미지
	thumbnail: string;
	/* 
    ...
    MVT인 경우 Vector이므로 OpenLayers layer 설정 시 new ol.layer.VectorTile로 설정할 것. 
    */
	mapType: "XYZ" | "MVT";
	category: "세계지도" | "육도" | "공도" | "해도";
	// D2MapServer에서 가져올 지도 데이터 소스
	url: string;
	// MVT일 경우에만 필요
	mvtUrl?: string;
	// 최소 줌 레벨
