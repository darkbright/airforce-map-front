	...
	init: function( editor ) {
		...
		var controls = {
			lineheight: new CKEDITOR.spacingControl( 
				{
					name: 'lineheight', 
					min: getConfig( CKEDITOR.config.lineheight_min, 1 ),
					max: getConfig( CKEDITOR.config.lineheight_max, 3 ),
					step: getConfig( CKEDITOR.config.lineheight_step, 0.5 )
				},
				editor ),
			letterspacing: new CKEDITOR.spacingControl( 
				{
					name: 'letterspacing', 
					min: getConfig( CKEDITOR.config.letterspacing_min, 0 ), // -20
					max: getConfig( CKEDITOR.config.letterspacing_max, 5 ), // 20
					step: getConfig( CKEDITOR.config.letterspacing_step, 1)
				},
				editor )
		};
...
CKEDITOR.config.spacingsliders_lineheightStyle = {
	...
	getStyleValue: function( path, block ) {
		...
		return size ? Math.floor( value / size ) : 0;
	}
};
